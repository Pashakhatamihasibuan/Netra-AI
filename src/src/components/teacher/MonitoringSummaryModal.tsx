...
// ── Badge + click untuk guru di tabel hasil ────────────────────────────────
export interface MonitoringRowSummary {
  studentId: string;
  totalWarnings: number;
  byReason: Record<string, number>;
  questionCount: number;
...
export function MonitoringWarningBadge({
  row,
  onClick,
}: {
  row: MonitoringRowSummary | undefined;
  onClick: () => void;
}) {
  if (!row) return <span className="text-gray-300 text-xs">—</span>;
  if (row.totalWarnings === 0)
    return <Badge tone="safe">✓ Aman</Badge>;
  return (
    <button
      type="button"
      onClick={onClick}
      className="inline-flex items-center gap-1 rounded-full bg-amber-50 border border-amber-200 px-2 py-0.5 text-xs font-semibold text-amber-800 hover:bg-amber-100 transition-colors"
    >
      ⚠️ {row.totalWarnings} dtk warning
    </button>
  );
}

// ── Modal detail guru ──────────────────────────────────────────────────────
interface MonitoringSummaryModalProps {
  quizId: string;
...
export function MonitoringSummaryModal({ quizId, student, onClose }: MonitoringSummaryModalProps) {
  ...
  // Realtime: update saat siswa masih mengerjakan
  useEffect(() => {
    if (!student) return;
    const supabase = createClient();
      ...
      <div className="w-full max-w-2xl bg-white rounded-3xl shadow-2xl max-h-[85vh] flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">Monitoring per soal · {rows.length} soal tercatat</p>
          </div>
          <button
            onClick={onClose}
...
// ── Hook: ambil ringkasan semua siswa untuk 1 kuis ─────────────────────────
export function useMonitoringSummary(quizId: string | null) {
  ...
  const load = useCallback(async () => {
    ...
    // FIX: reset dulu ke {} agar tidak ada jeda di mana results sudah ada
    // tapi summary masih berisi data kuis sebelumnya -> undefined access
    setSummary({});
    const data = await authedFetch(`/api/teacher/monitoring?quizId=${quizId}`);
    const map: Record<string, MonitoringRowSummary> = {};
    for (const s of (data.summary ?? []) as SummaryRow[]) {
      map[s.studentId] = {
  ...
  // FIX: return { summary } agar bisa di-destructure:
  // const { summary: monitoringSummary } = useMonitoringSummary(...)
  // Sebelumnya hook return `summary` langsung (bukan object),
  // sehingga destructuring { summary } = ... menghasilkan undefined
  return { summary };
}

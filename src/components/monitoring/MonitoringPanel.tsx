...
function buildWarnings(t: (section: any, key: string) => string, params: {
  ...
  if (params.distanceCm !== null) {
    if (params.distanceCm < 30) w.push(t('monitoring', 'w_too_close'));
    else if (params.distanceCm > 70) w.push(t('monitoring', 'w_too_far'));
  }
  if (params.lighting === 'dark')   w.push(t('monitoring', 'w_dark'));
  if (params.lighting === 'bright') w.push(t('monitoring', 'w_bright'));
  if (params.posture === 'poor')    w.push(t('monitoring', 'w_posture'));
  else if (params.posture === 'warning') w.push(t('monitoring', 'w_posture_warn'));
  if (params.blinkRate > 0 && params.blinkRate < 8)
    w.push(t('monitoring', 'w_blink'));

  return w;
}

// POSTURE_LABEL is now via t() inline

// ── Draggable camera widget ───────────────────────────────────────────────────

function DraggableCameraWidget({ faceMesh, handleStart, warnings, postureTone, t }: {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between bg-[#0D2B1E] rounded-t-xl px-2 py-1 cursor-grab active:cursor-grabbing">
        <span className="text-[10px] text-white/60 font-semibold tracking-wide select-none">{t('monitoring', 'cam_label')}</span>
        <div className="flex items-center gap-1">
          <button
            ...
            title={minimized ? t('monitoring', 'expand') : t('monitoring', 'minimize')}
          >
            {minimized ? '▲' : '▼'}
          </button>
        </div>
          ...
          {faceMesh.isRunning && (
            <div className="px-2 py-1.5 flex flex-wrap gap-1">
              ...
              <Badge tone={postureTone as 'safe' | 'warning' | 'alert' | 'neutral'}>
                {faceMesh.posture === 'good' ? t('monitoring', 'posture_good') : faceMesh.posture === 'warning' ? t('monitoring', 'posture_warn') : t('monitoring', 'posture_poor')}
              </Badge>
            </div>
          )}
          {warnings.length > 0 && (
...
export function MonitoringPanel({ compact = false }: MonitoringPanelProps) {
  ...
  const warnings = buildWarnings(t, {
  ...
  if (compact) {
    return <DraggableCameraWidget faceMesh={faceMesh} handleStart={handleStart} warnings={warnings} postureTone={postureTone} t={t} />;
  }
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex-1 min-w-[260px]">
          ...
          <div className="flex gap-2 mt-3 flex-wrap">
            {!faceMesh.isRunning ? (
              <Button onClick={handleStart}>{t('monitoring', 'start_session')}</Button>
            ) : (
              <>
          ...
          {savedThisSession && (
            <p className="text-sm text-teal-600 mt-2">
              ✓ Sesi disimpan.{' '}
              {newBadges.length > 0 && (
                <span className="font-medium">{t('monitoring', 'badge_new')}{newBadges.join(', ')} 🎉</span>
              )}
            </p>
          )}
        </div>
        ...
        <div className="flex-1 min-w-[260px] space-y-3">
          ...
          {faceMesh.isRunning && warnings.length === 0 && (
            <div className="rounded-xl2 bg-teal-50 border border-teal-200 px-4 py-2">
              <p className="text-sm text-teal-700">{t('monitoring', 'all_safe')}</p>
            </div>
          )}

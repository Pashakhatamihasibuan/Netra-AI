  ...
  return (
    <div className="space-y-4">
      ...
      {rows.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: t('monitoring', 'stat_total_q'),         value: rows.length.toString(),               tone: 'neutral' as const },
            { label: t('monitoring', 'stat_warned_q'),    value: warnedQuestions.toString(),           tone: (warnedQuestions > 0 ? 'alert' : 'safe') as 'alert' | 'safe' },
            { label: t('monitoring', 'stat_total_warn'), value: totalWarningSeconds.toString(),       tone: (totalWarningSeconds > 0 ? 'warning' : 'safe') as 'warning' | 'safe' },
            { label: t('monitoring', 'stat_safe_q'),          value: (rows.length - warnedQuestions).toString(), tone: 'safe' as const },
          ].map(({ label, value, tone }) => (
      ...
      {loading ? (
        <div className="flex items-center gap-2 py-10 text-gray-400 text-sm">
          ...
          {t('monitoring', 'loading_detail') || 'Memuat data per soal…'}
        </div>
      ) : rows.length === 0 ? (
        <Card>
          <div className="text-center py-8">
            <p className="text-3xl mb-2">📭</p>
            <p className="text-sm text-gray-400">{t('monitoring', 'no_detail')}</p>
        ...
        <div className="space-y-2">
          <p className="text-xs text-gray-400 px-1">
            {t('monitoring', 'detail_hint') || 'Klik soal untuk melihat detail jarak & postur.'}
          </p>

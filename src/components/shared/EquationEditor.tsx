...
const TABS: TabGroup[] = [
  {
    ...
    items: [
      ...
      { sym: 'xⁿ',         label: 'Pangkat',          fields: [{id:'x',hint:'Bilangan dasar'},{id:'n',hint:'Eksponen / pangkat'}], tpl: v=>`${v.x}^{${v.n}}` },
  ...
  },
  {
    ...
    items: [
      ...
      { sym: 'Σ',   label: 'Sigma / Jumlah',   fields: [{id:'start',hint:'Mulai, mis: i=1'},{id:'end',hint:'Sampai, mis: n'}], tpl: v=>`\\sum_{${v.start}}^{${v.end}}` },
    ],
  },
  {
    label: 'Geometri',
    items: [
      { sym: 'Luas ○',      label: 'Luas lingkaran',        fields: [{id:'r',hint:'Jari-jari, mis: r atau 7'}], tpl: v=>`\\pi ${v.r}^{2}` },
      { sym: 'Keliling ○',  label: 'Keliling lingkaran',    fields: [{id:'r',hint:'Jari-jari, mis: r'}], tpl: v=>`2\\pi ${v.r}` },
      ...
      { sym: 'Vol. kubus',  label: 'Volume kubus',          fields: [{id:'s',hint:'Rusuk / sisi'}], tpl: v=>`${v.s}^{3}` },
  ...
  },
  {
    label: 'Statistik',
    items: [
      { sym: 'x̄ Rerata',  label: 'Rumus rerata',   fields: [{id:'n',hint:'Jumlah data, mis: n'}], tpl: v=>`\\bar{x}=\\frac{\\sum x}{${v.n}}` },
...
interface EquationEditorProps {
  ...
  onInsert?: (latex: string) => void; // callback saat tombol "Sisipkan ke soal" ditekan
  label?: string;
  showInsertButton?: boolean; // tampilkan tombol "Sisipkan ke soal" (default: false)
}
  ...
  return (
    <>
      <div className="rounded-2xl border border-gray-200 bg-white overflow-hidden">
          ...
          <div className="flex gap-0.5 px-2 pt-2 overflow-x-auto scrollbar-none">
            {TABS.map((g, i) => (
              <button
                ...
                className={`whitespace-nowrap px-2.5 py-1 rounded-t-lg text-xs font-medium transition-colors ${
                  activeTab === i
                    ? 'bg-white text-[#1B8A5A] border border-b-0 border-gray-200 shadow-sm'
        ...
        {value.trim() && (
          <div className="mx-3 mb-3 px-3 py-2.5 rounded-xl bg-[#F0FAF5] border border-[#D1EDE0]">
            ...
            <div className="text-base text-[#0D2B1E] overflow-x-auto">
              <MathRenderer latex={value} display />
            </div>
          </div>
        )}
      </div>
      ...
      {modalItem && (
        <FieldModal
          item={modalItem}
          onConfirm={(latex) => { insertAt(latex); setModalItem(null); }}
...
// Drop-in replacement — API sama persis dengan versi lama

interface MathEquationInsertProps {

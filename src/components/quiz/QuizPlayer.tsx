...
interface QuizPlayerProps { quizId: string; quizTitle: string; secondsPerQuestion?: number; randomizeOrder?: boolean }
...
export function QuizPlayer({ quizId, quizTitle, secondsPerQuestion, randomizeOrder = true }: QuizPlayerProps) {
  ...
  const sampleBufferRef  = useRef<Array<{ distanceCm: number | null; posture: string | null; lighting: string | null; blinkRate: number | null; hasWarning: boolean; warningReason: string | null }>>([]);
  ...
  useEffect(() => {
    ...
    async function startThenLoad() {
      setLoading(true);
      try {
        const startRes  = await fetch('/api/quiz/attempt/start', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ quizId }) });
        const startData = await startRes.json();
        if (cancelled) return;
        if (startData.blocked) { setBlocked({ reason: startData.reason, message: startData.message, score: startData.score ?? null }); setLoading(false); return; }
        attemptIdRef.current = startData.attemptId ?? null;
      } catch {
        if (!cancelled) { setBlocked({ reason: 'error', message: t('quiz', 'player_conn_err') }); setLoading(false); }
        return;
      }
      const supabase = createClient();
      const { data } = await supabase.from('questions_for_student').select('*').eq('quiz_id', quizId).order('order_index');
      if (cancelled) return;
  ...
  function flushQuestionMonitoring(questionIndex: number) {
    ...
    fetch('/api/quiz/monitoring-log', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ quizId, attemptId: attemptIdRef.current, questionIndex, answeredAt: new Date().toISOString(), samples }) }).catch(() => {});
  }
  ...
  useEffect(() => {
    ...
    function handleVisibilityChange() {
      if (document.hidden) {
        ...
        setBlocked({ reason: 'forfeited', message: t('quiz', 'player_forfeit_tab') });
        fetch('/api/quiz/attempt/forfeit', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ quizId }) }).catch(() => {});
      }
    }
  ...
  useEffect(() => {
    ...
    function handleFullscreenChange() {
      if (!document.fullscreenElement && !submitting) {
        flushQuestionMonitoring(index);
        setBlocked({ reason: 'forfeited', message: t('quiz', 'player_forfeit_fs') });
        fetch('/api/quiz/attempt/forfeit', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ quizId }) }).catch(() => {});
      }
    }
  ...
  async function handleStartQuiz() {
    ...
    try { const el = document.documentElement; if (el.requestFullscreen) await el.requestFullscreen(); } catch {}
  }
  ...
  let warningMessage = '';
  if (isBadPosture)   warningMessage = t('monitoring', 'w_posture_quiz');
  else if (isTooClose) warningMessage = t('monitoring', 'w_too_close_quiz');
  else if (isTooFar)   warningMessage = t('monitoring', 'w_too_far_quiz');
  ...
  useEffect(() => {
    ...
    const tick = setInterval(() => { sampleBufferRef.current.push({ distanceCm, posture, lighting, blinkRate, hasWarning, warningReason }); }, 1000);
  ...
  async function submitQuiz() {
    ...
    setSubmitting(false);
    if (!res.ok) { setBlocked({ reason: 'forfeited', message: data.error ?? t('quiz', 'player_cannot_submit'), score: data.score ?? 0 }); return; }
    setResult(data);
  }
  ...
  if (blocked) {
    ...
    return (
      <Card className="text-center">
        ...
        {blocked.score !== null && blocked.score !== undefined && <p className="text-3xl font-display font-bold text-teal-600 mt-2">{blocked.score}</p>}
  ...
  if (result) {
    return (
      <Card>
        ...
        <p className="text-sm text-ink/70">{t('quiz', 'player_correct').replace('{correct}', String(result.correctCount)).replace('{total}', String(result.totalQuestions))}</p>
      </Card>
    );
  }
  ...
  if (!current) return <Card>{t('quiz', 'player_no_questions')}</Card>;
  ...
  if (!started) {
    return (
      <Card className="text-center">
        ...
        {secondsPerQuestion && <p className="text-sm text-ink/50 mb-4">{t('quiz', 'player_timed_msg')}</p>}
        ...
        <div><Button onClick={handleStartQuiz}>{t('quiz', 'player_start')}</Button></div>
      </Card>
    );
  }
      ...
      <div className={`transition-opacity duration-300 ${isPaused ? 'opacity-20 blur-sm pointer-events-none' : 'opacity-100'}`}>
        <div className="flex justify-between items-center mb-2">
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1 bg-cream rounded-lg px-2 py-1 border border-teal-100">
              <button onClick={() => setFontSize((s) => Math.max(14, s - 2))} className="w-6 h-6 flex items-center justify-center text-ink/60 hover:text-ink rounded transition-colors font-bold text-sm">A−</button>
              <span className="text-xs text-ink/40 px-1 select-none">{fontSize}px</span>
              <button onClick={() => setFontSize((s) => Math.min(28, s + 2))} className="w-6 h-6 flex items-center justify-center text-ink/60 hover:text-ink rounded transition-colors font-bold text-sm">A+</button>
            </div>
        ...
        <div className="space-y-2">
          {(['A', 'B', 'C', 'D'] as const).map((opt) => (
            <button key={opt} onClick={() => selectAnswer(opt)} disabled={isPaused} style={{ fontSize }}
              className={`w-full text-left rounded-xl2 border px-4 py-3 transition-colors ${selected === opt ? 'border-teal-600 bg-teal-50' : 'border-teal-50 hover:bg-cream'}`}>
